  ...
  PostCollection.create(newPost, async (err, createdPost) => {
    if (err) {
      ...
      try {
        ...
        createdPost = await UserCollection.populate(createdPost, {
          path: "postedBy",
          select: "profilePic firstName lastName username _id",
        });
        createdPost = await PostCollection.populate(createdPost, {
          path: "replyTo",
        });
        createdPost = await UserCollection.populate(createdPost, {
...
export const getPost = async (req, res) => {
  ...
  try {
    ...
    postFromDb = await UserCollection.populate(postFromDb, {
      path: "retweetData.postedBy",
      select: "profilePic firstName lastName username _id",
    });
    postFromDb = await UserCollection.populate(postFromDb, {
...
export const getUserPosts = async (req, res) => {
  ...
  try {
    ...
    let postsFromDb = await PostCollection.find({postedBy:userId})
      .populate({
        path: "postedBy",
        select: "profilePic firstName lastName username _id",
      })
...
export const retweetPost = async (req, res) => {
  const postId = req.params.id;
  try {
    let retweetPost = await PostCollection.create({
      retweetData: postId,
      postedBy: req.userId,
    ...
    retweetPost = await UserCollection.populate(retweetPost, {
      path: "postedBy",
      select: "profilePic firstName lastName username _id",
    });
    retweetPost = await PostCollection.populate(retweetPost, {
      path: "retweetData",
    });
    retweetPost = await UserCollection.populate(retweetPost, {
      path: "retweetData.postedBy",
...
export const unretweetPost = async (req, res) => {
  ...
  try {
    ...
    await PostCollection.findByIdAndUpdate(postId, {
      $pull: { retweetUsers: req.userId },
    });
    const deletedRetweet = await PostCollection.findOneAndDelete({
      postedBy: req.userId,
      retweetData: postId,
    });
